...
def do_zip_update():
    ...
    if success:
        mesgdcrt.SuccessMessage("RRPS was updated to the latest version")
        mesgdcrt.GeneralMessage(
            "Please run the script again to load the latest version")
    else:
        mesgdcrt.FailureMessage("Unable to update RRPS")
        mesgdcrt.WarningMessage(
            "Grab The Latest one From https://github.com/riturajpratapsingh/bomber.git")
...
def do_git_update():
    ...
    if success:
        ...
        print(
            "git checkout . && "
            "git pull https://github.com/riturajpratapsingh/bomber.git HEAD")
    sys.exit()
...
def check_for_updates():
    ...
    fver = requests.get(
            "https://raw.githubusercontent.com/riturajpratapsingh/bomber/master/.version"
            ).text.strip()
    if fver != __VERSION__:
        ...
        mesgdcrt.SuccessMessage("RRPS is up-to-date")
        mesgdcrt.GeneralMessage("Starting RRPS")


def notifyen():
    try:
        noti = requests.get(
            "https://raw.githubusercontent.com/riturajpratapsingh/bomber/master/.notify"
            ).text.upper()
        if len(noti) > 10:
...
def pretty_print(cc, target, success, failed):
    ...
    mesgdcrt.SuccessMessage("RRPS was created by Ritu Raj Pratap Singh")
...
if sys.version_info[0] != 3:
    mesgdcrt.FailureMessage("RRPS will work only in Python v3")
    sys.exit()
...
__CONTRIBUTORS__ = ['RRPS']

ALL_COLORS = [Fore.GREEN, Fore.RED, Fore.YELLOW, Fore.BLUE,
              Fore.MAGENTA, Fore.CYAN, Fore.WHITE]
RESET_ALL = Style.RESET_ALL

description = """RRPS - Your Friendly Spammer Application

RRPS can be used for many purposes which incudes -
...
parser = argparse.ArgumentParser(description=description,
                                 epilog='Coded by Ritu Raj Pratap Singh !!!')
parser.add_argument("-sms", "--sms", action="store_true",
                    help="start RRPSwith SMS Bomb mode")
parser.add_argument("-call", "--call", action="store_true",
                    help="start RRPS with CALL Bomb mode")
parser.add_argument("-mail", "--mail", action="store_true",
                    help="start RRPS with MAIL Bomb mode")
parser.add_argument("-u", "--update", action="store_true",
                    help="update TBomb")
parser.add_argument("-c", "--contributors", action="store_true",
                    help="show current RRPS contributors")
parser.add_argument("-v", "--version", action="store_true",
                    help="show current RRPS version")
